...
const CreateProjectModal: React.FC<CreateProjectModalProps> = ({ openModal, setOpenModal, loadProjects }) => {
  ...
  const handleValidateName = (value: string = projectName ?? '') => {
    const required = validateInput('name', value ?? '', PROJECT_VALIDATE_ENUM.NAME_EMPTY, APP_VALIDATE_TYPE.REQUIRED, validateError, setValidateError);
    if (!required) {
      return false;
    }
    return true;
      ...
      <ModalBody>
        <div className="row mb-2">
          ...
          <div className="col-12 mt-2">
            <Textarea maxLength={290} rows={3} placeholder="Description about project" id="projectDescription" ref={descriptionRef}></Textarea>
          </div>
