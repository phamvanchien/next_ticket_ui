...
import TimeTrackingDetailView from "@/views/time-tracking-detail/TimeTrackingDetailView";
...
const TimeTrackingDetail: React.FC<TimeTrackingDetailPageProps> = async ({ params }) => {
  return <ComingSoonView />
  // const memberTracking = await fetchTrackingAdmin(params.id, params.tracking_id);

  // if (!memberTracking || memberTracking.code !== API_CODE.OK) {
  //   return <ErrorPage code={memberTracking.code} />;
  // }

  // return <TimeTrackingDetailView timeTracking={memberTracking.data} />
}

const fetchTrackingAdmin = async (workspaceId: number, trackingId: number): Promise<BaseResponseType<TimeTrackingType>> => {
  try {
    const apiResponse = await fetch(APP_CONFIG.API.URL + APP_CONFIG.API.PREFIX.time_tracking.url + '/' + workspaceId.toString() + '/' + trackingId, {
